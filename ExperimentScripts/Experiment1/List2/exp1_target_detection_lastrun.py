...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\amnesia\\Desktop\\Misc\\clean_up\\right\\experiments_final\\list_2\\exp1_target_detection_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    thisExp.addLoop(session_1)  # add the loop to the experiment
    thisSession_1 = session_1.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisSession_1.rgb)
    if thisSession_1 != None:
        for paramName in thisSession_1:
            globals()[paramName] = thisSession_1[paramName]
    if thisSession is not None:
        # if running in a Session with a Liaison client, send data up to now
        thisSession.sendExperimentData()
    
    for thisSession_1 in session_1:
        currentLoop = session_1
        thisExp.timestampOnFlip(win, 'thisRow.t', format=globalClock.format)
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
        ...
        # --- Run Routine "fix" ---
        # if trial has changed, end Routine now
        if isinstance(session_1, data.TrialHandler2) and thisSession_1.thisN != session_1.thisTrial.thisN:
            continueRoutine = False
        fix.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            # get current time
        ...
        if key_control2.keys in ['', [], None]:  # No response was made
            key_control2.keys = None
        session_1.addData('key_control2.keys',key_control2.keys)
        if key_control2.keys != None:  # we had a response
            session_1.addData('key_control2.rt', key_control2.rt)
            session_1.addData('key_control2.duration', key_control2.duration)
        # the Routine "fix" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        ...
        if key_resp.keys in ['', [], None]:  # No response was made
            key_resp.keys = None
        session_1.addData('key_resp.keys',key_resp.keys)
        if key_resp.keys != None:  # we had a response
            session_1.addData('key_resp.rt', key_resp.rt)
            session_1.addData('key_resp.duration', key_resp.duration)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if target.maxDurationReached:
            routineTimer.addTime(-target.maxDuration)
        elif target.forceEnded:
    ...
    # completed 1.0 repeats of 'session_1'
    ...
    while continueRoutine:
        ...
        if tThisFlip > break_2.maxDuration-frameTolerance:
            break_2.maxDurationReached = True
            continueRoutine = False
    ...
    thisExp.addLoop(session_2)  # add the loop to the experiment
    thisSession_2 = session_2.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisSession_2.rgb)
    if thisSession_2 != None:
        for paramName in thisSession_2:
            globals()[paramName] = thisSession_2[paramName]
    if thisSession is not None:
        # if running in a Session with a Liaison client, send data up to now
        thisSession.sendExperimentData()
    
    for thisSession_2 in session_2:
        currentLoop = session_2
        thisExp.timestampOnFlip(win, 'thisRow.t', format=globalClock.format)
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
        ...
        # --- Run Routine "fix" ---
        # if trial has changed, end Routine now
        if isinstance(session_2, data.TrialHandler2) and thisSession_2.thisN != session_2.thisTrial.thisN:
            continueRoutine = False
        fix.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            # get current time
        ...
        if key_control2.keys in ['', [], None]:  # No response was made
            key_control2.keys = None
        session_2.addData('key_control2.keys',key_control2.keys)
        if key_control2.keys != None:  # we had a response
            session_2.addData('key_control2.rt', key_control2.rt)
            session_2.addData('key_control2.duration', key_control2.duration)
        # the Routine "fix" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        ...
        if key_resp.keys in ['', [], None]:  # No response was made
            key_resp.keys = None
        session_2.addData('key_resp.keys',key_resp.keys)
        if key_resp.keys != None:  # we had a response
            session_2.addData('key_resp.rt', key_resp.rt)
            session_2.addData('key_resp.duration', key_resp.duration)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if target.maxDurationReached:
            routineTimer.addTime(-target.maxDuration)
        elif target.forceEnded:
    ...
    # completed 1.0 repeats of 'session_2'
